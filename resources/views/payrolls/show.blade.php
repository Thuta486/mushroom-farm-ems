        ...
        <div class="rounded-xl border border-stone-200 bg-white p-6">
            ...
            <dl class="mt-4 space-y-3 text-sm">
                <div class="flex justify-between gap-4">
                    ...
                    <dd class="font-medium text-stone-900">{{ $payroll->total_worked_days }} {{ __('app.common.days') }}
                    </dd>
                </div>
                <div class="flex justify-between gap-4">
                ...
                <div class="flex justify-between gap-4">
                    <dt class="text-red-600">{{ __('app.payrolls.attendance_deduction') }}</dt>
                    <dd class="font-medium text-red-700">
                        @if ($payroll->payrollAdjustments->isNotEmpty())
                            {{ number_format($payroll->attendance_deduction, 0) }}
                ...
                <div class="flex justify-between gap-4 border-t border-stone-200 pt-3">
                    ...
                    <dd class="font-medium text-stone-900">{{ number_format($payroll->employee->wage_amount, 0) }}
                        {{ __('app.common.currency') }}</dd>
                </div>
            </dl>
        </div>
        ...
        <div class="rounded-xl border border-stone-200 bg-white p-6">
            ...
            <dl class="mt-4 space-y-3 text-sm">
                <div class="flex justify-between gap-4">
                    ...
                    <dd class="font-medium text-stone-900">{{ number_format($payroll->gross_salary, 0) }}
                        {{ __('app.common.currency') }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-emerald-600">{{ __('app.payrolls.bonuses') }}</dt>
                    <dd class="font-medium text-emerald-700">+ {{ number_format($payroll->total_bonus, 0) }}
                        {{ __('app.common.currency') }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-red-600">{{ __('app.payrolls.deductions') }}</dt>
                    <dd class="font-medium text-red-700">− {{ number_format($payroll->total_deduction, 0) }}
                        {{ __('app.common.currency') }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-red-600">{{ __('app.payrolls.cash_advances') }}</dt>
                    <dd class="font-medium text-red-700">− {{ number_format($payroll->total_advances, 0) }}
                        {{ __('app.common.currency') }}</dd>
                </div>
                <div class="flex justify-between gap-4 border-t border-stone-200 pt-3">
                    <dt class="font-semibold text-stone-900">{{ __('app.payrolls.net_pay') }}</dt>
                    <dd class="text-lg font-semibold text-stone-900">{{ number_format($payroll->net_pay, 0) }}
                        {{ __('app.common.currency') }}</dd>
                </div>
            </dl>
        </div>
                ...
                <table class="min-w-[900px] w-full divide-y divide-stone-200">
                    <thead class="bg-stone-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-stone-500">
                                {{ __('app.common.category') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-stone-500">
                                {{ __('app.common.amount') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-stone-500">
                                {{ __('app.common.reason') }}</th>
                            @unless ($isPaid)
                                <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wide text-stone-500">
                                    {{ __('app.common.actions') }}</th>
                            @endunless
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-stone-100">
                        @forelse ($payroll->payrollAdjustments as $adjustment)
                            <tr>
                                <td class="px-6 py-4 text-sm text-stone-900">
                                    {{ $adjustment->adjustmentType->display_name }}
                                </td>
                                <td class="px-6 py-4 text-sm text-stone-600">
                                    {{ __('app.adjustment_types.category_' . $adjustment->adjustmentType->category) }}
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-stone-900">
                                    {{ number_format($adjustment->amount, 0) }} {{ __('app.common.currency') }}</td>
                                ...
                                @unless ($isPaid)
                                    <td class="px-6 py-4 text-right text-sm">
                                        <form method="POST"
                                            ...
                                            class="inline"
                                            onsubmit="return confirm('{{ __('app.payrolls.remove_adjustment_confirmation') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="font-medium text-red-600 hover:text-red-700">{{ __('app.common.remove') }}</button>
                                        </form>
                                    </td>
                                @endunless
                            </tr>
                        @empty
                            <tr>
                                <td colspan="{{ $isPaid ? 4 : 5 }}" class="px-6 py-8 text-center text-sm text-stone-500">
                                    {{ __('app.payrolls.no_adjustments_yet') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @unless ($isPaid)
                <div class="border-t border-stone-200 px-6 py-4">
                    <form method="POST" action="{{ route('payrolls.adjustments.store', $payroll) }}"
                        ...
                        @csrf
                        <x-form-select name="adjustment_type_id" label="{{ __('app.common.type') }}" :options="$adjustmentTypes->pluck('display_name', 'id')"
                            placeholder="{{ __('app.cash_advances.select_type') }}" />
                        <x-form-input name="amount" label="{{ __('app.common.amount') }} ({{ __('app.common.currency') }})"
