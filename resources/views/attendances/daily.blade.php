...
@section('title', __('app.attendance.daily_attendance'))
...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-stone-900">{{ __('app.attendance.daily_attendance') }}</h1>
            <p class="mt-1 text-sm text-stone-500">{{ __('app.attendance.mark_instructions') }}</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <x-button href="{{ route('attendances.daily', ['date' => $previousDate]) }}" variant="secondary">{{ __('app.attendance.previous_day') }}</x-button>
            <x-button href="{{ route('attendances.daily', ['date' => $nextDate]) }}" variant="secondary">{{ __('app.attendance.next_day') }}</x-button>
            <x-button href="{{ route('attendances.index') }}" variant="secondary">{{ __('app.attendance.view_history') }}</x-button>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('attendances.daily') }}" class="mb-6 flex flex-col gap-4 rounded-xl border border-stone-200 bg-white p-4 sm:flex-row sm:items-end">
        <x-form-input name="date" label="{{ __('app.attendance.attendance_date') }}" type="date" :value="$date->toDateString()" class="sm:max-w-xs" />
        <x-button type="submit" variant="secondary">{{ __('app.attendance.go_to_date') }}</x-button>
    </form>

    @if ($employees->isEmpty())
        <div class="rounded-xl border border-stone-200 bg-white p-8 text-center text-sm text-stone-500">
            {{ __('app.attendance.no_active_employees') }}
        </div>
    @else
        <form method="POST" action="{{ route('attendances.daily.store') }}">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-stone-200">
                        <thead class="bg-stone-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-stone-500 sm:px-6">{{ __('app.attendance.employee') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-stone-500 sm:px-6">{{ __('app.attendance.department') }}</th>
            ...
            <div class="mt-6 flex justify-end">
                <x-button type="submit">{{ __('app.attendance.save') }}</x-button>
            </div>
        </form>
    @endif
