...
@section('content')
    ...
    <form method="GET" action="{{ route('reports.cash-advances') }}" class="mb-6 grid gap-4 rounded-xl border border-stone-200 bg-white p-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        <div class="flex items-end gap-2 md:col-span-2">
            ...
            <x-button href="{{ route('reports.cash-advances') }}" variant="secondary" class="w-full">{{ __('app.common.reset') }}</x-button>
        </div>
    </form>
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-2">
        ...
        <div class="rounded-xl border border-stone-200 bg-white p-5">
            ...
            <p class="mt-1 text-2xl font-semibold text-emerald-700">{{ number_format($report['summary']->total_amount ?? 0, 0) }} {{ __('app.common.currency') }}</p>
        </div>
    </div>
    ...
    <table class="min-w-[900px] w-full divide-y divide-stone-200">
            <thead class="bg-stone-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-stone-500">{{ __('app.common.total_amount') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-stone-100">
                @forelse ($report['employeeRows'] as $row)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-stone-600">{{ $row->department_name ?? __('app.employees.no_department') }}</td>
                        <td class="px-6 py-4 text-sm text-stone-600">{{ $row->advance_count }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-stone-900">{{ number_format($row->total_amount, 0) }} {{ __('app.common.currency') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-sm text-stone-500">{{ __('app.cash_advances.no_cash_advances_found') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <table class="min-w-[900px] w-full divide-y divide-stone-200">
            ...
            <tbody class="divide-y divide-stone-100">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-sm text-stone-500">{{ __('app.cash_advances.no_cash_advances_found') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
